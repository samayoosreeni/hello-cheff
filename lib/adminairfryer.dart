...
class adminairfryer extends StatefulWidget {
  const adminairfryer({super.key});

  @override
  State<adminairfryer> createState() => _adminairfryerState();
}

class _adminairfryerState extends State<adminairfryer> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Airfryer').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Airfryer',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Airfryer')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminairfryer', arguments: {
...
class airfryerupdate extends StatefulWidget {
  const airfryerupdate({super.key});

  @override
  State<airfryerupdate> createState() => _airfryerupdateState();
}

class _airfryerupdateState extends State<airfryerupdate> {
  ...
  String image = '';
  String selectedCategory = 'Airfryer';
  late String docId;
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Airfryer';
    docId = args['id'];
  }

  Future<void> updateairfryer({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Airfryer', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await updateairfryer(category: selectedCategory);

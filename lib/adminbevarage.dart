...
class adminbevarage extends StatefulWidget {
  const adminbevarage({super.key});

  @override
  State<adminbevarage> createState() => _adminbevarageState();
}

class _adminbevarageState extends State<adminbevarage> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Bevarage').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Bevarage',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Bevarage')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminbevarage', arguments: {
...
class bevarageupdate extends StatefulWidget {
  const bevarageupdate({super.key});

  @override
  State<bevarageupdate> createState() => _bevarageupdateState();
}

class _bevarageupdateState extends State<bevarageupdate> {
  ...
  String image = '';
  String selectedCategory = 'Bevarage';
  late String docId;
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Bevarage';
    docId = args['id'];
  }

  Future<void> updatebevarage({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Beverage', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatebevarage(category: selectedCategory);

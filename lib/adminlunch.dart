...
class adminlunch extends StatefulWidget {
  const adminlunch({super.key});

  @override
  State<adminlunch> createState() => _adminlunchState();
}

class _adminlunchState extends State<adminlunch> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Lunch').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Lunch',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Lunch')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminlunch', arguments: {
...
class lunchupdate extends StatefulWidget {
  const lunchupdate({super.key});

  @override
  State<lunchupdate> createState() => _lunchupdateState();
}

class _lunchupdateState extends State<lunchupdate> {
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Lunch';
    docId = args['id'];
  }

  Future<void> updatelunch({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Lunch', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatelunch(category: selectedCategory);

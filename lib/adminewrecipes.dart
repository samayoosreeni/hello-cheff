...
class adminewrecipes extends StatefulWidget {
  const adminewrecipes({super.key});

  @override
  State<adminewrecipes> createState() => _adminewrecipesState();
}

class _adminewrecipesState extends State<adminewrecipes> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('NewRecipes').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('New recipes',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('NewRecipes')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminewrecipes', arguments: {
...
class newrecipesupdate extends StatefulWidget {
  const newrecipesupdate({super.key});

  @override
  State<newrecipesupdate> createState() => _newrecipesupdateState();
}

class _newrecipesupdateState extends State<newrecipesupdate> {
  ...
  String image = '';
  String selectedCategory = 'NewRecipes';
  late String docId;
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'NewRecipes';
    docId = args['id'];
  }

  Future<void> updatenewrecipes({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update NewRecipes', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: <String>[
                  ...
                  // 'Bevarage',
                   'NewRecipes',
                  // 'NonVeg', 'Vegan', 'Airfryer', 'NoCook', 'Desserts', 'Seafood', 'Kids'
                ].map((String value) {
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatenewrecipes(category: selectedCategory);

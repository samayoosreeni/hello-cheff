...
class adminnonveg extends StatefulWidget {
  const adminnonveg({super.key});

  @override
  State<adminnonveg> createState() => _adminnonvegState();
}

class _adminnonvegState extends State<adminnonveg> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('NonVeg').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Non-Veg',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('NonVeg')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'admin nonveg', arguments: {
...
class Nonvegupdate extends StatefulWidget {
  const Nonvegupdate({super.key});

  @override
  State<Nonvegupdate> createState() => _NonvegupdateState();
}

class _NonvegupdateState extends State<Nonvegupdate> {
  ...
  String image = '';
  String selectedCategory = 'NonVeg';
  late String docId;
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'NonVeg';
    docId = args['id'];
  }

  Future<void> updatenonveg({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update NonVeg', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: <String>[
                  ...
                  // 'Bevarage','NewRecipes',
                  'NonVeg',
                  // 'Vegan', 'Airfryer', 'NoCook', 'Desserts', 'Seafood', 'Kids'
                ].map((String value) {
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatenonveg(category: selectedCategory);

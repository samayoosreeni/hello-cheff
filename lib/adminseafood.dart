...
class adminseafood extends StatefulWidget {
  const adminseafood({super.key});

  @override
  State<adminseafood> createState() => _adminseafoodState();
}

class _adminseafoodState extends State<adminseafood> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Seafood').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Seafood',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Seafood')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminseafood', arguments: {
...
class seafoodupdate extends StatefulWidget {
  const seafoodupdate({super.key});

  @override
  State<seafoodupdate> createState() => _seafoodupdateState();
}

class _seafoodupdateState extends State<seafoodupdate> {
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Seafood';
    docId = args['id'];
  }

  Future<void> updateseafood({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Seafood', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: <String>[
                  // 'None', 'Breakfast', 'Lunch', 'Dinner', 'Bevarage','NewRecipes', 'NonVeg','Vegan', 'Airfryer', 'NoCook','Desserts',
                  'Seafood',
                  // 'Kids'
                ].map((String value) {
              ...
              ElevatedButton(
                onPressed: () async {
                  await updateseafood(category: selectedCategory);

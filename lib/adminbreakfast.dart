...
class adminbreak extends StatefulWidget {
  const adminbreak({super.key});

  @override
  State<adminbreak> createState() => _adminbreakState();
}

class _adminbreakState extends State<adminbreak> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Breakfast').doc(docId).delete();
                          ...
                          trailing: Container(
                            ...
                            child: Row(
                              children: [
                                IconButton(
                                    onPressed: () {
                                      Navigator.pushNamed(context, 'adminbreakfast', arguments: {
...
class breakupdate extends StatefulWidget {
  const breakupdate({super.key});

  @override
  State<breakupdate> createState() => _breakupdateState();
}

class _breakupdateState extends State<breakupdate> {
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Breakfast';
    docId = args['id'];
  }

  Future<void> updateFood({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Breakfast', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: <String>[
                  // 'None',
                  'Breakfast',
                  // 'Lunch', 'Dinner', 'Beverage', 'NewRecipes',
              ...
              ElevatedButton(
                onPressed: () async {
                  await updateFood(category: selectedCategory);

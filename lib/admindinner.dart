...
class admindinner extends StatefulWidget {
  const admindinner({super.key});

  @override
  State<admindinner> createState() => _admindinnerState();
}

class _admindinnerState extends State<admindinner> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Dinner').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Dinner',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Dinner')
              .snapshots(),
    ...
    Navigator.pushNamed(context, 'admindinner', arguments: {
...
class dinnerupdate extends StatefulWidget {
  const dinnerupdate({super.key});

  @override
  State<dinnerupdate> createState() => _dinnerupdateState();
}

class _dinnerupdateState extends State<dinnerupdate> {
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Dinner';
    docId = args['id'];
  }

  Future<void> updatedinner({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Dinner', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatedinner(category: selectedCategory);

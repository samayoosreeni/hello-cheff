...
class adminnocook extends StatefulWidget {
  const adminnocook({super.key});

  @override
  State<adminnocook> createState() => _adminnocookState();
}

class _adminnocookState extends State<adminnocook> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('NoCook').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('No-Cook',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('NoCook')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'adminnocook', arguments: {
...
class nocookupdate extends StatefulWidget {
  const nocookupdate({super.key});

  @override
  State<nocookupdate> createState() => _nocookupdateState();
}

class _nocookupdateState extends State<nocookupdate> {
  ...
  String image = '';
  String selectedCategory = 'NoCook';
  late String docId;
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'NoCook';
    docId = args['id'];
  }

  Future<void> updatenocook({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update NoCook', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatenocook(category: selectedCategory);

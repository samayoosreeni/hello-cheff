...
class adminkids extends StatefulWidget {
  const adminkids({super.key});

  @override
  State<adminkids> createState() => _adminkidsState();
}

class _adminkidsState extends State<adminkids> {
  Future<void> _deleteItem(String docId) async {
    try {
      await FirebaseFirestore.instance.collection('Kids').doc(docId).delete();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Kids',style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold),),
      ),
        body: StreamBuilder(
          stream:  FirebaseFirestore.instance.collection('Kids')
              .snapshots(),
                            ...
                            child: Row(
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.pushNamed(context, 'admindkids', arguments: {
...
class kidsupdate extends StatefulWidget {
  const kidsupdate({super.key});

  @override
  State<kidsupdate> createState() => _kidsupdateState();
}

class _kidsupdateState extends State<kidsupdate> {
  ...
  void didChangeDependencies() {
    ...
    selectedCategory = args['Category'] ?? 'Kids';
    docId = args['id'];
  }

  Future<void> updatekids({required String category}) async {
    try {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          title: Text('Update Kids', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: <String>[
                  // 'None', 'Breakfast', 'Lunch', 'Dinner', 'Bevarage','NewRecipes', 'NonVeg','Vegan', 'Airfryer', 'NoCook','Desserts',// 'Seafood',
                  'Kids'
                ].map((String value) {
              ...
              ElevatedButton(
                onPressed: () async {
                  await updatekids(category: selectedCategory);
